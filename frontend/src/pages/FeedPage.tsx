...
export default function FeedPage() {
  ...
  // Simulate an async API fetch with try-catch
  useEffect(() => {
    let cancelled = false;
    ...
    const fetchBooks = async () => {
      ...
      setError(null);
      try {
        // Simulate network latency (replace with real API call e.g. api.get('/posts'))
  ...
  const filtered = useMemo(() => {
    return books.filter(b => {
      ...
      const matchesYear =
        !yearFilter || String((b as unknown as Record<string, unknown>).year ?? "").includes(yearFilter);
      const matchesRating = !minRating || b.rating >= parseFloat(minRating);
      const matchesPrice =
        !maxPrice ||
        ((b as unknown as Record<string, unknown>).price !== undefined &&
          ((b as unknown as Record<string, number>).price) <= parseFloat(maxPrice));
